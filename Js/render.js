...
export function renderGame(ctx, notesToRender, currentSettings, images, time, triggeredNotes) {
    ...
    for (let i = notesToRender.length - 1; i >= 0; i--) {
        ...
        if (_t >= (currentSettings.distanceToMid - 2) / currentSettings.speed && _t < (note.holdTime ?? 0)) {
            ...
            if (note.star) {
                drawStar(currentX, currentY, currentSize, color, (nang) / -4 * Math.PI, ctx, hbw, currentSettings, calAng, noteBaseSize);
                ...
                drawHold(currentX, currentY, currentSize, color, (nang) / -4 * Math.PI, holdLength, ctx, hbw, currentSettings, calAng, noteBaseSize);
            } else {
                drawTap(currentX, currentY, currentSize, color, ctx, hbw, currentSettings, noteBaseSize);
            }
        }
    }
...
export function drawTap(x, y, sizeFactor, color, ctx, hbw, currentSettings, noteBaseSize) {
...
export function drawStar(x, y, sizeFactor, color, ang, ctx, hbw, currentSettings, calAng, noteBaseSize) {
...
export function path(type, startNp, endNp, hw, hh, hbw, calAng) {
    ...
    switch (type) {
        ...
        case '>': {
            let turn = (startNp >= 1 && startNp <= 4); // Assuming startNp is 1-8 from simai, convert to 0-7 for logic if needed
...
export function drawHold(x, y, sizeFactor, color, ang, l, ctx, hbw, currentSettings, calAng, noteBaseSize) {
    ang = ang - 0.125 * Math.PI;
    let s = noteBaseSize;
    let currentSize = Math.max(sizeFactor * s, 0);
