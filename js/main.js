...
let example = `(180){4}1-5[4:1],2,3,4,
`;
...
function tap(pos, t, r, no) {
    ...
    if (no.break) {
        ...
        let find = note.indexOf(note.find((element) => element.time == no.time && element.starTime == null));
        if (note[find + 1] != null) {
...
function star(pos, poe, t, typ, no) {
    ...
    let angS = (0 - poe) / 8 * 2 * Math.PI + Math.PI / 8;
    let angE = (0 - pos) / 8 * 2 * Math.PI + Math.PI / 8;
    let midX = ctx.canvas.width / 2;
    let midY = ctx.canvas.height / 2;
    ...
    switch (typ) {
        case '-':
            ...
            break;
        case '^':
            ctx.arc(midX, midY, 0 - d, angS - Math.PI / 4, angE - Math.PI / 4);
            break;
        default:
            break;
    }
